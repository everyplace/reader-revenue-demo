...
import {oneMonthFromNow} from '../app/routes/subscription-linking/entitlements.js';
...
function safeEnvVars() {
  ...
  const SWG_JS_URL =
    SWG_URL ||
    `https://news.google.com/swg/js/v1/swg${
      SWG_OVERRIDE ? `-${SWG_OVERRIDE}` : ''
    }.js`;

  const SWG_JS_MJS_URL =
    process.env.SWG_MJS_URL ||
    `https://news.google.com/swg/js/v1/swg${
      SWG_OVERRIDE ? `-${SWG_OVERRIDE}` : ''
    }.mjs`;

  const SWG_JS_GAA_URL =
    process.env.SWG_GAA_URL ||
    `https://news.google.com/swg/js/v1/swg-gaa${
      SWG_OVERRIDE ? `-${SWG_OVERRIDE}` : ''
    }.js`;

  const SWG_JS_GAA_MJS_URL =
    process.env.SWG_GAA_MJS_URL ||
    `https://news.google.com/swg/js/v1/swg-gaa${
      SWG_OVERRIDE ? `-${SWG_OVERRIDE}` : ''
    }.mjs`;

  const SWG_JS_BASIC_URL =
    process.env.SWG_BASIC_URL ||
    `https://news.google.com/swg/js/v1/swg-basic${
      SWG_OVERRIDE ? `-${SWG_OVERRIDE}` : ''
    }.js`;

  const SWG_JS_BASIC_MJS_URL =
    process.env.SWG_BASIC_MJS_URL ||
    `https://news.google.com/swg/js/v1/swg-basic${
      SWG_OVERRIDE ? `-${SWG_OVERRIDE}` : ''
    }.mjs`;

  const SWG_JS_PUBLISHER_URL =
    process.env.SWG_PUBLISHER_URL ||
    `https://news.google.com/swg/js/v1/publisher${
      SWG_OVERRIDE ? `-${SWG_OVERRIDE}` : ''
    }.js`;

  const SWG_JS_PUBLISHER_MJS_URL =
    process.env.SWG_PUBLISHER_MJS_URL ||
    `https://news.google.com/swg/js/v1/publisher${
      SWG_OVERRIDE ? `-${SWG_OVERRIDE}` : ''
    }.mjs`;
  ...
  return {
    ...
    SWG_JS_GAA_MJS_URL,
    SWG_JS_BASIC_URL,
    SWG_JS_BASIC_MJS_URL,
    SWG_JS_PUBLISHER_URL,
    SWG_JS_PUBLISHER_MJS_URL,
    PAY_SWG_VERSION,
    NEWSLETTER_CTA_CONFIGURATION_ID,
    SWG_SKU,
    OTHER_SKU1,
...
hbs.handlebars.registerHelper("includes", (str, substr) => str.includes(substr));
...
async function renderMarkdown(filePath, template, data) {
  ...
  try {
    ...
    // passing a sample value for `expire_time` in the Subscription Linking demo
    data.oneMonthFromNow = oneMonthFromNow();
...
async function renderStaticFile(filePath, data = {}) {
  try {
    ...
    const pattern = /\bprocess\.env\.([a-zA-Z0-9_]+)\b/g;
    const replacedStaticFile = staticFile.replace(
      pattern,
      (match, paramName) => {
